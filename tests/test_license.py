...
def test_cache_optimization_valid(mock_env):
    ...
    data = {
        ...
        "plan_tier": "pro",
        "signature": _get_signature(last_verified, "cached_key", "pro")
    }
...
def test_cache_optimization_expired(mocker, mock_env):
    ...
    data = {
        ...
        "last_verified": last_verified,
        "plan_tier": "pro",
        "signature": _get_signature(last_verified, "expired_key", "pro")
    }
    cache_file.parent.mkdir(parents=True, exist_ok=True)
    with open(cache_file, "w") as f:
        json.dump(data, f)
...
def test_ci_bypasses_cache(mocker, monkeypatch):
    ...
    # Mock urllib response returning activated: True with team tier
    mock_urlopen = mocker.patch('urllib.request.urlopen')
    mock_response = mocker.MagicMock()
    mock_response.read.return_value = json.dumps({
        "activated": True,
    ...
    data = {
        ...
        "last_verified": last_verified,
        "plan_tier": "team",
        "signature": _get_signature(last_verified, "cached_key", "team")
    }
    cache_file.parent.mkdir(parents=True, exist_ok=True)
    with open(cache_file, "w") as f:
        json.dump(data, f)

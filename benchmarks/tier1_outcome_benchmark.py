...
def generate_markdown_report(data: Dict[str, Any]) -> str:
    ...
    if data.get("status") == "BENCHMARK NOT RUN":
        reason = data.get("reason", "API credentials not configured or live probe failed.")
        runs_per_task = data.get("runs_per_task", 5)
        total_evals = data.get("total_evaluations_planned", len(DEVELOPER_TASKS) * runs_per_task * 3)
        ...
        lines = [
            ...
            "",
            f"Planned total evaluations: **{total_evals} trials** (10 tasks × {runs_per_task} runs × 3 context modes across 4 difficulty tiers).",
            "",
